...
def test_change_is_parseable_by_parse_hunks() -> None:
    # The generated diff must round-trip through the parser so the preview can
    # render it with diffview.render_hunk.
    original = "# Title\n\nold line\n"
    edited = "# Title\n\nnew line\n"
...
def test_multiline_replacement() -> None:
    ...
    edited = "a\nB2\nB3\nc\n"
    diff = build_unified_diff(original, edited)
    texts = [line.text for hunk in parse_hunks(diff) for line in hunk.lines]
