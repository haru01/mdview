...
本文をマウスでドラッグ選択してから `a` を押すとポップアップが開きます。質問を入力して `Enter` を押すと、選択した抜粋とあわせて `claude -p` に渡され、回答が表示されます（`Esc` で閉じる）。

`claude` は閲覧中ファイルのリポジトリルート（`.git` を持つ最寄りの親ディレクトリ、なければファイルのあるディレクトリ）を作業ディレクトリとして実行されるため、リポジトリ内の関連ファイルを踏まえた回答が得られます。
