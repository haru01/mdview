...
- **AI に質問（Ask AI）** — 本文をマウスで選択して `h` を押すと、選択範囲について `claude` に質問できる（別途 `claude` CLI が必要、後述）。
...
本文をマウスでドラッグ選択してから `h` を押すとポップアップが開きます。質問を入力して `Enter` を押すと、開いているドキュメントの全文と選択した抜粋・質問が `claude -p` に渡され、回答が表示されます（`Esc` で閉じる）。
